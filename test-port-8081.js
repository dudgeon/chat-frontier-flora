...
async function testPort8081() {
  console.log('🚀 Testing if app is actually running on port 8081...');
  ...
  try {
    ...
    await page.screenshot({ path: 'port-8081-test.png' });
    console.log('📸 Screenshot saved as port-8081-test.png');
    ...
    try {
      const response = await page.goto('http://localhost:8081/', { waitUntil: 'domcontentloaded', timeout: 3000 });
      console.log('📡 Server response status:', response.status());
    } catch (e) {
      console.log('❌ Server not responding on port 8081');
    }
  } finally {
    await browser.close();
  }
