...
async function testPort8082() {
  console.log('🚀 Testing if app is actually running on port 8082...');
  ...
  try {
    console.log('📱 Testing http://localhost:8082/...');
    await page.goto('http://localhost:8082/', { waitUntil: 'domcontentloaded', timeout: 10000 });
    ...
    await page.screenshot({ path: 'port-8082-test.png' });
    console.log('📸 Screenshot saved as port-8082-test.png');
    ...
    try {
      const response = await page.goto('http://localhost:8082/', { waitUntil: 'domcontentloaded', timeout: 3000 });
      console.log('📡 Server response status:', response.status());
    } catch (e) {
      console.log('❌ Server not responding on port 8082');
    }
  } finally {
    await browser.close();
  }
